...
export default function MetricsPage({ vehicles, history }: MetricsPageProps) {
  ...
  return (
    <div className="page-shell">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '15px', marginBottom: '30px' }}>
        <div style={{ backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '12px', borderLeft: `5px solid ${SEMANTIC_TEXT.ok}`, boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
          ...
          <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
            {Math.round((grafanaData.summary.uso / (grafanaData.summary.total || 1)) * 100)}% da frota em campo
          </span>
        </div>
        ...
        <div style={{ backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '12px', borderLeft: `5px solid #3498db`, boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '8px' }}>
            <span>Higienização / Lavador</span>
            <Droplet size={18} color="#3498db" />
          </div>
          <div style={{ fontSize: '1.8rem', fontWeight: 800 }}>{grafanaData.summary.lav} no lavador</div>
          <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Padrão diário de sanitização</span>
        </div>
      </div>

      {/* SECTION 1: RANKING DE USO (Dia, Semana, Mês, Ano) */}
      <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '22px', marginBottom: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: '20px' }}>
          <div>
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Classificação por quantidade de retiradas e quilometragem no período</p>
          </div>

          <div style={{ display: 'flex', backgroundColor: 'var(--bg-main)', padding: '4px', borderRadius: '8px', border: '1px solid var(--border)' }}>
            {(['day', 'week', 'month', 'year'] as const).map(p => (
              ...
              >
                {p === 'day' ? 'Hoje (Dia)' : p === 'week' ? 'Semana' : p === 'month' ? 'Mês' : 'Ano'}
              </button>
            ))}
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
            ...
            <tbody>
              {usageRanking.map((item, idx) => (
                <tr key={item.id} style={{ borderBottom: '1px solid var(--border)', backgroundColor: idx % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.02)' }}>
                  <td style={{ padding: '12px 10px', fontWeight: 700 }}>
                    {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx + 1}º`}
                  </td>
                  <td style={{ padding: '12px 10px', fontWeight: 700, color: 'var(--brand-primary)' }}>{item.label}</td>
                  <td style={{ padding: '12px 10px' }}>{item.sublabel}</td>
                  <td style={{ padding: '12px 10px' }}><strong>{item.withdrawalsCount}</strong> saídas</td>
                  <td style={{ padding: '12px 10px' }}><strong>{item.totalKm.toLocaleString()}</strong> km</td>
      ...
      <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '22px', marginBottom: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Clock size={20} color="var(--brand-secondary)" />
          Tempo de Inoperância e Estado do Veículo
        </h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '20px' }}>
          Monitoramento do tempo sem uso (estacionado em pátio), tempo acumulado em manutenção e dias sem higienização.
        </p>
            ...
            <tbody>
              {timeMetrics.map(item => (
                <tr key={item.vehicleId} style={{ borderBottom: '1px solid var(--border)' }}>
                  <td style={{ padding: '12px 10px', fontWeight: 700 }}>
                    {item.tag} <span style={{ fontWeight: 400, color: 'var(--text-secondary)' }}>({item.plate})</span>
                  </td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        {/* Peak Hours Chart */}
        <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '22px', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', height: '160px', paddingTop: '20px' }}>
            {peakUsage.hours.map(h => {
              ...
              return (
                <div key={h.hourNumber} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: h.count === maxPeakHourCount && h.count > 0 ? '#e67e22' : 'var(--brand-primary)',
                      borderRadius: '4px 4px 0 0',
                      transition: 'all 0.3s'
                    }}
                  />
                  <span style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', transform: 'rotate(-45deg)', transformOrigin: 'top left', marginTop: '4px' }}>
        ...
        <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '22px', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            <div>
              <h4 style={{ fontSize: '0.9rem', color: 'var(--brand-secondary)', marginBottom: '10px' }}>👨‍✈️ Top Motoristas</h4>
              {kmRanking.drivers.slice(0, 5).map((d, i) => (
                <div key={d.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: '0.85rem' }}>
                  <span>{i + 1}. {d.name}</span>
                  <strong>{d.km.toLocaleString()} km</strong>
                </div>
              ))}
              {kmRanking.drivers.length === 0 && <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Sem histórico de viagens registrado.</p>}
            </div>

            <div>
              <h4 style={{ fontSize: '0.9rem', color: 'var(--brand-primary)', marginBottom: '10px' }}>🚘 Veículos Mais Rodados</h4>
              {kmRanking.vehicles.slice(0, 5).map((v, i) => (
                <div key={v.tag} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: '0.85rem' }}>
                  <span>{i + 1}. {v.tag}</span>
      ...
      <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '22px', marginBottom: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'grid', gap: '10px', maxHeight: '320px', overflowY: 'auto' }}>
          {obsLogs.map(log => (
            <div key={log.id} style={{ borderLeft: '4px solid var(--brand-secondary)', backgroundColor: 'var(--bg-main)', padding: '12px 16px', borderRadius: '0 8px 8px 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                <span><strong>{log.vehicleTag}</strong> • {log.author} ({log.action})</span>
                <span>{log.date}</span>
              </div>
          ...
          {obsLogs.length === 0 && <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Nenhuma observação registrada.</p>}
        </div>
      </div>
