...
export default function RegionaisPage({
  ...
  const unassignedVehicles = useMemo(
    () =>
      regionalVehicles.filter(
        (v) => !v.gerenciaId || !regionalGerencias.some((g) => g.id === v.gerenciaId)
      ),
    ...
    <div className="page-shell">
          ...
          isAdmin && (
            <>
              <button
                ...
                onClick={() => setShowGerenciaModal(true)}
              >
      ...
      {!regional ? (
        ...
        <div style={{ display: 'grid', gap: 'var(--space-5)' }}>
          ...
          >
            <StatCard
              label="Veículos na regional"
              value={regionalVehicles.length}
              icon={<Truck size={17} />}
            />
            <StatCard
              label="Gerências"
              value={regionalGerencias.length}
            ...
            <StatCard
              ...
              hint={
                unassignedVehicles.length
                  ? 'Fora de auditoria setorial'
                  : 'Toda a frota está vinculada'
              }
              icon={<Hash size={17} />}
                    ...
                    <Card key={g.id} padding="md">
                      ...
                      <div style={{ display: 'grid', gap: 'var(--space-2)', fontSize: '0.85rem' }}>
                        ...
                        >
                          <UserRound size={14} />
                          Responsável:{' '}
                          <strong style={{ color: 'var(--text-primary)' }}>{g.responsible}</strong>
                        </p>
                        <p
                          style={{
                            display: 'flex',
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          style={{
                            fontSize: '0.75rem',
                            color: gerencia ? 'var(--text-muted)' : 'var(--state-alert)',
                          }}
                        >
        ...
        <form onSubmit={handleCreateRegional} style={{ display: 'grid', gap: 'var(--space-4)' }}>
          ...
          <div>
            ...
            <input
              ...
              className="field"
              required
              placeholder="Ex.: REG-SUL"
              value={regCode}
              onChange={(e) => setRegCode(e.target.value)}
            />
          </div>
          <div>
            <label htmlFor="reg-desc" className="field-label">
              Descrição
            </label>
            <textarea
              id="reg-desc"
              className="field"
              rows={3}
              placeholder="Área de atuação e escopo operacional"
              value={regDesc}
      ...
      >
        <form onSubmit={handleCreateGerencia} style={{ display: 'grid', gap: 'var(--space-4)' }}>
          ...
          <div>
            <label htmlFor="ger-regional" className="field-label">
              Regional
            </label>
            <select
              id="ger-regional"
              ...
              required
              value={gerRegionalId}
              onChange={(e) => setGerRegionalId(e.target.value)}
            >
              {regionais.map((r) => (
          ...
          <div>
            <label htmlFor="ger-name" className="field-label">
              Nome da gerência
            </label>
            <input
              id="ger-name"
              ...
              value={gerName}
              onChange={(e) => setGerName(e.target.value)}
            />
          </div>
          <div>
            <label htmlFor="ger-code" className="field-label">
              Código
            </label>
            <input
              id="ger-code"
              className="field"
              required
              placeholder="Ex.: GER-OPS"
              value={gerCode}
              onChange={(e) => setGerCode(e.target.value)}
            />
          </div>
          <div>
            <label htmlFor="ger-resp" className="field-label">
              Responsável
            </label>
            <input
              id="ger-resp"
              ...
              value={gerResp}
              onChange={(e) => setGerResp(e.target.value)}
            />
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-2)' }}>
            <button
              type="button"
              className="btn btn-outline"
              onClick={() => setShowGerenciaModal(false)}
            >
              Cancelar
            </button>
