...
export function calculateUsageRanking(
  ...
  const map: Record<
    string,
    { withdrawals: number; km: number; activeMinutes: number; vehicle: Vehicle | null }
  > = {};
...
export function calculateVehicleTimeMetrics(vehicles: Vehicle[]): VehicleIdleMetric[] {
  ...
  return vehicles.map((v) => {
    const statusDate = v.lastStatusChangeAt
      ? new Date(v.lastStatusChangeAt)
      : new Date(now.getTime() - 48 * 3600 * 1000);
    const washedDate = v.lastWashedAt
      ? new Date(v.lastWashedAt)
      : new Date(now.getTime() - 120 * 3600 * 1000);

    const diffHours = Math.max(
      0,
      Math.round((now.getTime() - statusDate.getTime()) / (1000 * 3600))
    );
    const washedDiffDays = Math.max(
      0,
      Math.round((now.getTime() - washedDate.getTime()) / (1000 * 3600 * 24))
    );
    ...
    return {
      ...
      statusSinceFormatted: statusDate.toLocaleString('pt-BR', {
        day: '2-digit',
        month: '2-digit',
        hour: '2-digit',
        minute: '2-digit',
...
export function calculatePeakUsage(history: HistoryItem[]): {
  hours: PeakUsageTime[];
  days: PeakUsageDay[];
} {
...
export function calculateKmRanking(
  history: HistoryItem[],
  vehicles: Vehicle[]
): {
  vehicles: { tag: string; model: string; km: number }[];
  drivers: { name: string; km: number }[];
} {
...
export function extractObservationsTimeline(
  history: HistoryItem[],
  vehicles: Vehicle[]
): ObservationLog[] {
