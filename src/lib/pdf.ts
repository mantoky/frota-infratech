...
export function generateFleetReport(vehicles: Vehicle[], history: HistoryItem[]) {
  ...
  yPos += 10
  driverStats.filter(d => calculateDriverKm(d[0], history) > 0).forEach(([driverName]) => {
  ...
  if (history.length > 0) {
    autoTable(doc, {
      ...
      body: [...history].reverse().slice(0, 100).map(item => [item.date, item.vehicle, item.driver || '-', item.action, item.km.toLocaleString() + ' km', item.extra || '-']),
      theme: 'grid',
      headStyles: { fillColor: [52, 73, 94], fontSize: 8 },
