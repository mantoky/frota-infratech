...
import { Regional, Gerencia, ForumPost, ForumComment, ChecklistField, Vehicle } from '@/types';
import {
  initialRegionais,
  initialGerencias,
  initialForumPosts,
...
export function useOrgData() {
  ...
  useEffect(() => {
    ...
    if (backup) {
      ...
      setChecklistFields(
        backup.checklistFields?.length ? backup.checklistFields : initialChecklistFields
      );
    }
    setReady(true);
  ...
  const createGerencia = useCallback(
    (data: { regionalId: string; name: string; code: string; responsible: string }) => {
  ...
  const likeForumPost = useCallback(
    (postId: string) => {
      setForumPosts((prev) => {
